...
func (g *MakeGenerator2) generateMainMakefile() error {
	mk := cutils.NewLineWriter(cutils.IndentModeTabs)
...
func (g *MakeGenerator2) generateProjectMakefile(project *Project, isMain bool) {
	...
	mk.WriteLine(`#-------------------------------------------------------------------------------`)
	mk.WriteAlignedLine(`PRODUCT`, cutils.TabStop(0), `:= `, project.Name)
	mk.WriteAlignedLine(`PRODUCT_LIB`, cutils.TabStop(0), `:= lib`, project.Name)
	mk.WriteAlignedLine(`PRODUCT_DYLIB`, cutils.TabStop(0), `:= lib`, project.Name)
	mk.WriteAlignedLine(`PRODUCT_FRAMEWORK`, cutils.TabStop(0), `:= `, project.Name)
	mk.WriteAlignedLine(`PRODUCT_FRAMEWORK`, cutils.TabStop(0), `:= `, project.Name)

	// All include directories
	rootPath := filepath.Join(g.TargetAbsPath, project.Name)
	for _, cfg := range project.Resolved.Configs.Values {
		mk.WriteAligned(`INCLUDES_CPP_`, strings.ToLower(cfg.String()), cutils.TabStop(0), `:=`)
	...
	for _, cfg := range project.Resolved.Configs.Values {
		mk.WriteAligned(`LIBS_`, strings.ToLower(cfg.String()), cutils.TabStop(0), `:=`)
	...
	mk.WriteAlignedLine(`FLAGS_STD_C`, cutils.TabStop(0), `:= c99`)

	switch g.Workspace.Config.CppStd {
	case CppStd11:
		mk.WriteAlignedLine(`FLAGS_STD_CPP`, cutils.TabStop(0), `:= c++11`)
	case CppStd14:
		mk.WriteAlignedLine(`FLAGS_STD_CPP`, cutils.TabStop(0), `:= c++14`)
	case CppStd17:
		mk.WriteAlignedLine(`FLAGS_STD_CPP`, cutils.TabStop(0), `:= c++17`)
	case CppStd20:
		mk.WriteAlignedLine(`FLAGS_STD_CPP`, cutils.TabStop(0), `:= c++20`)
	case CppStdLatest:
		mk.WriteAlignedLine(`FLAGS_STD_CPP`, cutils.TabStop(0), `:= c++latest`)
	default:
		mk.WriteAlignedLine(`FLAGS_STD_CPP`, cutils.TabStop(0), `:= c++17`)
	}

	mk.WriteAlignedLine(`FLAGS_OTHER_`, cutils.TabStop(0), `:=`)
	mk.WriteAlignedLine(`FLAGS_C`, cutils.TabStop(0), `:=`)
	mk.WriteAlignedLine(`FLAGS_M`, cutils.TabStop(0), `:= -fobjc-arc`)
	mk.WriteAlignedLine(`FLAGS_MM`, cutils.TabStop(0), `:= -fobjc-arc`)
...
func (g *MakeGenerator2) generateProjectTargets(project *Project, isMain bool, mk *cutils.LineWriter) {
	...
	for _, dir := range directories {
		for _, f := range dir.Files {
			if !f.ExcludedFromBuild && (f.Is_C_or_CPP() || f.Is_ObjC()) {
				path := cutils.PathGetRelativeTo(dir.DiskPath, cutils.PathParent(project.ProjectAbsPath))
	...
	project.SrcFiles.Enumerate(isCppFileNotExcluded, func(i int, key string, value *FileEntry, last int) {
		path := cutils.PathGetRelativeTo(filepath.Join(project.ProjectAbsPath, value.Path), cutils.PathParent(project.ProjectAbsPath))
	...
	project.SrcFiles.Enumerate(isCFileNotExcluded, func(i int, key string, f *FileEntry, last int) {
		srcfile := cutils.PathGetRelativeTo(filepath.Join(project.ProjectAbsPath, f.Path), filepath.Join(g.TargetAbsPath, project.Name))
		buildfile := cutils.PathGetRelativeTo(filepath.Join(project.ProjectAbsPath, f.Path), cutils.PathParent(project.ProjectAbsPath))
