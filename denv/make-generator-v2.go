...
func (g *MakeGenerator2) generateProjectMakefile(project *Project, isMain bool) {
	...
	mk.WriteLine(`#-------------------------------------------------------------------------------`)
	mk.WriteAlignedLine(`PRODUCT`, foundation.TabStop(0), `:= `, project.Name)
	mk.WriteAlignedLine(`PRODUCT_LIB`, foundation.TabStop(0), `:= lib`, project.Name)
	mk.WriteAlignedLine(`PRODUCT_DYLIB`, foundation.TabStop(0), `:= lib`, project.Name)
	mk.WriteAlignedLine(`PRODUCT_FRAMEWORK`, foundation.TabStop(0), `:= `, project.Name)
	mk.WriteAlignedLine(`PRODUCT_FRAMEWORK`, foundation.TabStop(0), `:= `, project.Name)

	// All include directories
	rootPath := filepath.Join(g.TargetAbsPath, project.Name)
	for _, cfg := range project.Resolved.Configs.Values {
		mk.WriteAligned(`INCLUDES_CPP_`, strings.ToLower(cfg.String()), foundation.TabStop(0), `:=`)
	...
	if g.Workspace.Config.Dev.CompilerIsClang() {
		mk.WriteAlignedLine(`CC`, foundation.TabStop(0), `:= clang++`)
	} else if g.Workspace.Config.Dev.CompilerIsGcc() {
		mk.WriteAlignedLine(`CC`, foundation.TabStop(0), `:= gcc`)
	}

	// The compiler preprocessor defines
	for _, cfg := range project.Resolved.Configs.Values {
		mk.WriteAligned(`DEFINES_CPP_`, strings.ToLower(cfg.String()), foundation.TabStop(0), `:=`)
	...
	for _, cfg := range project.Resolved.Configs.Values {
		mk.WriteAligned(`LIBS_`, strings.ToLower(cfg.String()), foundation.TabStop(0), `:=`)
	...
	for _, cfg := range project.Resolved.Configs.Values {
		mk.WriteAligned(`FLAGS_WARN_`, strings.ToLower(cfg.String()), foundation.TabStop(0), `:=`)
	...
	for _, cfg := range project.Resolved.Configs.Values {
		mk.WriteAligned(`FLAGS_CPP_`, strings.ToLower(cfg.String()), foundation.TabStop(0), `:=`)
	...
	mk.WriteAlignedLine(`FLAGS_STD_C`, foundation.TabStop(0), `:= c99`)

	switch g.Workspace.Config.CppStd {
	case CppStd11:
		mk.WriteAlignedLine(`FLAGS_STD_CPP`, foundation.TabStop(0), `:= c++11`)
	case CppStd14:
		mk.WriteAlignedLine(`FLAGS_STD_CPP`, foundation.TabStop(0), `:= c++14`)
	case CppStd17:
		mk.WriteAlignedLine(`FLAGS_STD_CPP`, foundation.TabStop(0), `:= c++17`)
	case CppStd20:
		mk.WriteAlignedLine(`FLAGS_STD_CPP`, foundation.TabStop(0), `:= c++20`)
	case CppStdLatest:
		mk.WriteAlignedLine(`FLAGS_STD_CPP`, foundation.TabStop(0), `:= c++latest`)
	default:
		mk.WriteAlignedLine(`FLAGS_STD_CPP`, foundation.TabStop(0), `:= c++17`)
	}

	mk.WriteAlignedLine(`FLAGS_OTHER_`, foundation.TabStop(0), `:=`)
	mk.WriteAlignedLine(`FLAGS_C`, foundation.TabStop(0), `:=`)
	mk.WriteAlignedLine(`FLAGS_M`, foundation.TabStop(0), `:= -fobjc-arc`)
	mk.WriteAlignedLine(`FLAGS_MM`, foundation.TabStop(0), `:= -fobjc-arc`)
...
func (g *MakeGenerator2) generateProjectTargets(project *Project, isMain bool, mk *foundation.LineWriter) {
	...
	for _, dir := range directories {
		for _, f := range dir.Files {
			if f.Is_C_or_CPP() || f.Is_ObjC() {
				path := foundation.PathGetRelativeTo(dir.DiskPath, foundation.PathParent(project.ProjectAbsPath))
	...
	for _, group := range project.SrcFileGroups {
		group.Enumerate(isCppFileNotExcluded, func(i int, key string, value *FileEntry, last int) {
			path := foundation.PathGetRelativeTo(filepath.Join(group.Path, value.Path), foundation.PathParent(project.ProjectAbsPath))
	...
	for _, group := range project.SrcFileGroups {
		group.Enumerate(isCFileNotExcluded, func(i int, key string, f *FileEntry, last int) {
			srcfile := foundation.PathGetRelativeTo(filepath.Join(group.Path, f.Path), filepath.Join(g.TargetAbsPath, project.Name))
			buildfile := foundation.PathGetRelativeTo(filepath.Join(group.Path, f.Path), foundation.PathParent(group.Path))
	...
	for _, group := range project.SrcFileGroups {
		group.Enumerate(isObjCppFileNotExcluded, func(i int, key string, f *FileEntry, last int) {
			srcfile := foundation.PathGetRelativeTo(filepath.Join(group.Path, f.Path), filepath.Join(g.TargetAbsPath, project.Name))
			buildfile := foundation.PathGetRelativeTo(filepath.Join(group.Path, f.Path), foundation.PathParent(project.ProjectAbsPath))
			mk.WriteLine(`-include $(DIR_BUILD_TEMP)`, buildfile+`.d`)
			mk.WriteLine(`$(DIR_BUILD_TEMP)`, buildfile+`.o: `, srcfile)
			mk.WriteILine(`+`, `@echo -e $(call PRINT,compiling objective-c++,`, buildfile, `)`)
