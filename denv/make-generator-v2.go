...
func (g *MakeGenerator2) generateProjectMakefile(project *Project, isMain bool) {
	...
	mk.WriteLine(`#-------------------------------------------------------------------------------`)
	mk.WriteAlignedLine(`PRODUCT`, utils.TabStop(0), `:= `, project.Name)
	mk.WriteAlignedLine(`PRODUCT_LIB`, utils.TabStop(0), `:= lib`, project.Name)
	mk.WriteAlignedLine(`PRODUCT_DYLIB`, utils.TabStop(0), `:= lib`, project.Name)
	mk.WriteAlignedLine(`PRODUCT_FRAMEWORK`, utils.TabStop(0), `:= `, project.Name)
	mk.WriteAlignedLine(`PRODUCT_FRAMEWORK`, utils.TabStop(0), `:= `, project.Name)

	// All include directories
	rootPath := filepath.Join(g.TargetAbsPath, project.Name)
	for _, cfg := range project.Resolved.Configs.Values {
		mk.WriteAligned(`INCLUDES_CPP_`, strings.ToLower(cfg.String()), utils.TabStop(0), `:=`)
	...
	if g.Workspace.Config.Dev.CompilerIsClang() {
		mk.WriteAlignedLine(`CC`, utils.TabStop(0), `:= clang++`)
	} else if g.Workspace.Config.Dev.CompilerIsGcc() {
		mk.WriteAlignedLine(`CC`, utils.TabStop(0), `:= gcc`)
	}

	// The compiler preprocessor defines
	for _, cfg := range project.Resolved.Configs.Values {
		mk.WriteAligned(`DEFINES_CPP_`, strings.ToLower(cfg.String()), utils.TabStop(0), `:=`)
	...
	for _, cfg := range project.Resolved.Configs.Values {
		mk.WriteAligned(`FLAGS_WARN_`, strings.ToLower(cfg.String()), utils.TabStop(0), `:=`)
	...
	for _, cfg := range project.Resolved.Configs.Values {
		mk.WriteAligned(`FLAGS_CPP_`, strings.ToLower(cfg.String()), utils.TabStop(0), `:=`)
	...
	mk.WriteAlignedLine(`FLAGS_STD_C`, utils.TabStop(0), `:= c99`)

	switch g.Workspace.Config.CppStd {
	case CppStd11:
		mk.WriteAlignedLine(`FLAGS_STD_CPP`, utils.TabStop(0), `:= c++11`)
	case CppStd14:
		mk.WriteAlignedLine(`FLAGS_STD_CPP`, utils.TabStop(0), `:= c++14`)
	case CppStd17:
		mk.WriteAlignedLine(`FLAGS_STD_CPP`, utils.TabStop(0), `:= c++17`)
	case CppStd20:
		mk.WriteAlignedLine(`FLAGS_STD_CPP`, utils.TabStop(0), `:= c++20`)
	case CppStdLatest:
		mk.WriteAlignedLine(`FLAGS_STD_CPP`, utils.TabStop(0), `:= c++latest`)
	default:
		mk.WriteAlignedLine(`FLAGS_STD_CPP`, utils.TabStop(0), `:= c++17`)
	}

	mk.WriteAlignedLine(`FLAGS_OTHER_`, utils.TabStop(0), `:=`)
	mk.WriteAlignedLine(`FLAGS_C`, utils.TabStop(0), `:=`)
	mk.WriteAlignedLine(`FLAGS_M`, utils.TabStop(0), `:= -fobjc-arc`)
	mk.WriteAlignedLine(`FLAGS_MM`, utils.TabStop(0), `:= -fobjc-arc`)
...
func (g *MakeGenerator2) generateProjectTargets(project *Project, isMain bool, mk *utils.LineWriter) {
