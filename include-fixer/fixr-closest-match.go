...
func NewClosestMatch(possible []string, subsetSize []int) *ClosestMatch {
	cm := new(ClosestMatch)
...
func ClosestDistance(str1, str2 string) (int, float64) {
	...
	score := float64(len_s1 + len_s2)

	editDistance := column[len_s1]
	if editDistance == 0 {
		score = 1
	} else {
		score = (float64(score) - float64(editDistance)) / float64(score)
	}
