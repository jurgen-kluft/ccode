...
func (g *EspMakeGenerator) Generate() error {
	...
	// Create the files in the os directory
...
func (g *EspMakeGenerator) generateMakefile() error {
	...
	mk.WriteLine()
	mk.WriteLine(`START_TIME := $(shell date +%s)`)
	...
	if g.Workspace.MakeTarget.OSIsMac() {
		mk.WriteLine(`OS := macosx`)
	} else if g.Workspace.MakeTarget.OSIsWindows() {
		mk.WriteLine(`OS := windows`)
	} else {
		mk.WriteLine(`OS := linux`)
	}
	...
	mk.WriteLine(`UC_CHIP := $(shell perl -e "print uc $(CHIP)")`)
	if IS_ESP32 {
	...
	mk.WriteLine(`# ESP Arduino directories`)
	if DirExists(ESP_ROOT_ABS) {
		// mk.WriteLine(`  # Esp root not defined, find and use possible version in the Arduino IDE installation`)
		// mk.WriteLine(`  ARDUINO_ROOT ?= $(HOME)/.arduino15`)
		// mk.WriteLine(`  ARDUINO_ESP_ROOT = $(ARDUINO_ROOT)/packages/$(CHIP)`)
		// mk.WriteLine(`  ESP_ROOT := $(if $(ARDUINO_HW_ESP_ROOT),$(ARDUINO_HW_ESP_ROOT),$(lastword $(wildcard $(ARDUINO_ESP_ROOT)/hardware/$(CHIP)/*)))`)
		// mk.WriteLine(`  ifeq ($(ESP_ROOT),)`)
		// mk.WriteLine(`    $(error No installed version of $(CHIP) Arduino found)`)
		// mk.WriteLine(`  endif`)
		// mk.WriteLine(`  ARDUINO_PREFS = $(wildcard $(ARDUINO_ROOT)/preferences.txt)`)
		// mk.WriteLine(`  ifeq ($(ARDUINO_PREFS),)`)
		// mk.WriteLine(`    ARDUINO_LIBS ?= $(ARDUINO_ROOT)/libraries $(HOME)/Arduino/libraries`)
		// mk.WriteLine(`	else`)
		// mk.WriteLine(`    ARDUINO_LIBS ?= $(shell grep -o "sketchbook.path=.*" $(ARDUINO_PREFS) 2>/dev/null | cut -f2- -d=)/libraries`)
		// mk.WriteLine(`	endif`)
		// mk.WriteLine(`  ESP_ARDUINO_VERSION := $(notdir $(ESP_ROOT))`)
		// mk.WriteLine(`  # Find used version of compiler and tools`)
		// mk.WriteLine(`  COMP_PATH := $(lastword $(wildcard $(ARDUINO_ESP_ROOT)/tools/xtensa-*/*))`)
		// mk.WriteLine(`  MK_FS_PATH ?= $(lastword $(wildcard $(ARDUINO_ESP_ROOT)/tools/$(MK_FS_MATCH)/*/$(MK_FS_MATCH)))`)
		// mk.WriteLine(`  PYTHON3_PATH := $(lastword $(wildcard $(ARDUINO_ESP_ROOT)/tools/python3/*))`)
	} else {
		// mk.WriteLine(`else`)
		// mk.WriteLine(`  # Location defined, assume that it is a git clone`)
		// mk.WriteLine(`  ESP_ARDUINO_VERSION = $(call git_description,$(ESP_ROOT))`)
		// mk.WriteLine(`  MK_FS_PATH ?= $(lastword $(wildcard $(ESP_ROOT)/tools/$(MK_FS_MATCH)/$(MK_FS_MATCH)))`)
		// mk.WriteLine(`  PYTHON3_PATH := $(wildcard $(ESP_ROOT)/tools/python3)`)
		// mk.WriteLine(`endif`)
	}
	...
	arduinoEspGitInfo, _ := BuildGitInfo(ESP_ROOT, "git")
	ESP_ARDUINO_VERSION := arduinoEspGitInfo.Version

	mk.WriteLine(`# Location defined, assume that it is a git clone`)
	mk.WriteLine(`ESP_ARDUINO_VERSION = `, ESP_ARDUINO_VERSION)
	mk.WriteLine(`MK_FS_PATH ?= $(lastword $(wildcard $(ESP_ROOT)/tools/$(MK_FS_MATCH)/$(MK_FS_MATCH)))`)
	mk.WriteLine(`PYTHON3_PATH := $(wildcard $(ESP_ROOT)/tools/python3)`)
	...
	mk.WriteLine(`# Validate the selected version of ESP Arduino`)
	mk.WriteLine(`ifeq ($(wildcard $(ESP_ROOT)/cores/$(CHIP)),)`)
	mk.WriteLine(`  $(error $(ESP_ROOT) is not a valid directory for $(CHIP))`)
	mk.WriteLine(`endif`)

	mk.WriteLine(`# Validate the file system type`)
	mk.WriteLine(`ifeq ($(wildcard $(MK_FS_PATH)),)`)
	mk.WriteLine(`  $(error Invalid file system: "$(FS_TYPE)")`)
	mk.WriteLine(`endif`)
	...
	FLASH_DEF, _ := BoardOp(filepath.Join(ESP_ROOT, "boards.txt"), "esp32", "s3", "first_flash")
	// LWIP_VARIANT ?= $(shell $(BOARD_OP) $(BOARD) first_lwip)
	LWIP_VARIANT, _ := BoardOp(filepath.Join(ESP_ROOT, "boards.txt"), "esp32", "s3", "first_lwip")

	// Generate the Arduino makefile
	if err := GenerateArduinoMake(ESP_ROOT, "", BOARD_NAME, FLASH_DEF[0], OS_NAME, LWIP_VARIANT[0]); err != nil {
	...
	// TODO Add all the project base include directories

	C_INCLUDES := make([]string, 0)
	for _, dir := range INCLUDE_DIRS {
		C_INCLUDES = append(C_INCLUDES, fmt.Sprintf("-I%s", dir))
	}
	...
	mk.WriteLine(`	@perl -e 'print "Build complete. Elapsed time: ", time()-$(START_TIME),  " seconds\n\n"'`)
	...
	BOARD_LIST, _ := BoardOp(filepath.Join(ESP_ROOT_ABS, "boards.txt"), "esp32", "s3", "list_boards")
	...
	FLASH_LIST, _ := BoardOp(filepath.Join(ESP_ROOT_ABS, "boards.txt"), "esp32", "s3", "list_flash_defs")
	for _, flash := range FLASH_LIST {
		mk.WriteLine(`	@echo `, flash)
	}

	mk.WriteLine(`list_lwip:`)
	LWIP_LIST, _ := BoardOp(filepath.Join(ESP_ROOT_ABS, "boards.txt"), "esp32", "s3", "list_lwip")
