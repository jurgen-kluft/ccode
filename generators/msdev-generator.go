...
func (g *MsDevGenerator) genProjectFiles(wr *corepkg.XmlWriter, proj *Project) {
	tag := wr.TagScope("ItemGroup")
...
func (g *MsDevGenerator) genProjectPch(wr *corepkg.XmlWriter, proj *Project) {
	...
	tmp := corepkg.PathGetRelativeTo(proj.Resolved.PchHeader.Path, proj.Resolved.GeneratedFilesDir)

	proj.Resolved.PchHeader.Init(filename, true)
	code := "//-- Auto Generated File for Visual C++ precompiled header\n"
	code += "#include \"" + tmp + "\"\n"

	corepkg.WriteTextToFile(filename, code)
...
func (g *MsDevGenerator) genProjectConfig(wr *corepkg.XmlWriter, proj *Project, config *Config) {
	cond := "'$(Configuration)|$(Platform)'=='" + config.String() + "|" + g.VcxProjCpu + "'"
	{
		...
		wr.TagWithBody("OutDir", corepkg.PathNormalize(corepkg.PathGetRelativeTo(outDir, proj.GenerateAbsPath))+corepkg.PathSlash())
		wr.TagWithBody("IntDir", corepkg.PathNormalize(corepkg.PathGetRelativeTo(intDir, proj.GenerateAbsPath))+corepkg.PathSlash())
	...
	}
	{
		...
		}
		{
			...
			linkDirs, linkFiles, linkLibs := proj.BuildLibraryInformation(DevVisualStudio, config, g.Workspace.GenerateAbsPath)
		...
		if config.Resolved.OutputTarget.Path != "" {
			...
			{
				cmd := "mkdir -p \"" + "$(RemoteRootDir)/" + corepkg.PathDirname(config.Resolved.OutputTarget.Path) + "\";"
...
func (g *MsDevGenerator) genConfigOptionWithModifier(wr *corepkg.XmlWriter, name string, value *PinnedPathSet, modifier func(string, string, string) string) {
	option := value.Concatenated("", ";", modifier)
	option += "%(" + name + ")"
	wr.TagWithBody(name, option)
}

func (g *MsDevGenerator) writeSolutionProject(proj *Project, sb *corepkg.LineWriter) {
...
func (g *MsDevGenerator) genWorkspace(ws *ExtraWorkspace) {
	...
	}
	{
		...
		for _, c := range g.Workspace.ProjectGroups.Values {
			...
			c.MsDev.UUID = corepkg.GenerateUUID()

			catName := corepkg.PathFilename(c.Path, true)
	...
	wr := corepkg.NewXmlWriter()
	{
...

		{
			...
			for _, i := range proj.VirtualFolders.Folders {
				...
				winPath := corepkg.PathWindowsPath(i.Path)
