...
func (r *JsonDecoder) End() {
	r.Context.Json = ""
	r.Context.JsonLen = 0
	r.Context.Cursor = 0
	r.Context.StackIndex = jsonStackSize
	r.Context.EscapedStrings.Reset()
...
func (d *JsonDecoder) Decode(fields map[string]JsonDecode) error {
	...
	if !ok && d.Error == nil {
		return fmt.Errorf("error decoding JSON at %d", d.Context.Cursor)
	}
	return d.Error
}
...
func (d *JsonDecoder) DecodeStringArray() (outArray []string) {
	outArray = make([]string, 0, 4)
	ok, end := d.ReadUntilArrayEnd()
	for ok && !end {
		str := d.DecodeString()
		outArray = append(outArray, str)
		ok, end = d.ReadUntilArrayEnd()
	}
	return
}

// DecodeArray takes a callback to decode each element in the array
func (d *JsonDecoder) DecodeArray(decodeElement func(decoder *JsonDecoder)) {
	ok, end := d.ReadUntilArrayEnd()
	for ok && !end {
		decodeElement(d)
		ok, end = d.ReadUntilArrayEnd()
...
func (d *JsonDecoder) DecodeStringArray2D() (outArray [][]string) {
	outArray = make([][]string, 0, 4)
	ok, end := d.ReadUntilArrayEnd()
	for ok && !end {
		array := d.DecodeStringArray()
		outArray = append(outArray, array)
		ok, end = d.ReadUntilArrayEnd()
	}
	return
}

func (d *JsonDecoder) DecodeStringMapString() (outMap map[string]string) {
	outMap = make(map[string]string, 4)
	ok, end := d.ReadUntilObjectEnd()
	for ok && !end {
		key := d.DecodeField()
...
func (r *JsonDecoder) ReadUntilObjectEnd() (ok, end bool) {
	ok = r.Read()
	if !ok {
		return false, false
	}
	if r.Key.Type == JsonValueTypeObject && r.Value.Type == JsonValueTypeEnd {
		return true, true
	}
	return true, false
}

func (r *JsonDecoder) ReadUntilArrayEnd() (ok, end bool) {
	ok = r.Read()
	if !ok {
		return false, false
	}
	if r.Key.Type == JsonValueTypeArray && r.Value.Type == JsonValueTypeEnd {
		return true, true
	}
	return true, false
}

func (r *JsonDecoder) Read() (wasReadOk bool) {
	...
	case JsonValueTypeObject:
		okey, ovalue, oresult := r.Context.parseObjectBody()
		switch oresult {
	...
	case JsonValueTypeArray:
		...
		avalue, aresult := r.Context.parseArrayBody()
		switch aresult {
