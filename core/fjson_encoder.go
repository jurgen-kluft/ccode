...
func (e *JsonEncoder) HintOutputSize(size int) {
	e.json.Grow(size)
}
...
func (e *JsonEncoder) WriteStringArray(key string, values []string) {
	e.BeginArray(key)
	for _, v := range values {
		e.WriteArrayElement(v)
	}
	e.EndArray()
}

func (e *JsonEncoder) WriteStringArray2D(key string, values [][]string) {
	e.BeginArray(key)
	{
...
func (e *JsonEncoder) WriteArray(key string, len int, writeElementFun func(encoder *JsonEncoder, i int)) {
	e.BeginArray(key)
	i := 0
	for i < len {
		writeElementFun(e, i)
...
func (e *JsonEncoder) WriteAsArray(key string, writeFun func(encoder *JsonEncoder)) {
	e.BeginArray(key)
	writeFun(e)
	e.EndArray()
}

func (e *JsonEncoder) WriteStringMapString(key string, values map[string]string) {
	e.BeginMap(key)
	for k, v := range values {
		e.WriteFieldString(k, v)
	}
	e.EndMap()
}
