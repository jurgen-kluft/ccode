...
func TestInterpolateNew(t *testing.T) {
	...
	vars := NewVars(VarsFormatCurlyBraces)
	vars.Set("FOO", "String")
	vars.Set("BAR", "A", "B", "C")

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			resolver := NewVarResolver(VarsFormatDollarParenthesis)
...
func TestInterpolateNew2(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			resolver := NewVarResolver(VarsFormatCurlyBraces)
...
func TestNestedInterpolationNew(t *testing.T) {
	...
	result := vars.ResolveString("Test $(CCOPTS_$(CURRENT_VARIANT:u))", " ")
...
func TestNestedInterpolationExtensiveNew(t *testing.T) {
	...
	result := vars.ResolveString("Test $(CCOPTS_$(CURRENT_VARIANT:u):p__:s__)", " ")
	if len(result) != 1 {
		t.Errorf("expected length %d, got %d", 1, len(result))
	}
	if result != "Test __-g -O0__" {
