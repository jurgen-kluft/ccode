...
// The fileTracker is using the output of compilers like gcc,
...
func (d *fileTracker) Save() (int, error) {
...
func (d *fileTracker) CopyItem(item string) {
	d.hasher.Reset()
...
func (d *fileTracker) addItem(item string, extra []byte, deps []string) error {
...
func (d *fileTracker) AddItem(item string, deps []string) error {
...
func (d *fileTracker) QueryItem(item string) bool {
	d.hasher.Reset()
	d.hasher.Write([]byte(item))
...
func (d *fileTracker) AddItemWithExtraData(item string, data []byte, deps []string) error {
	return d.addItem(item, data, deps)
}

func (d *fileTracker) QueryItemWithExtraData(item string, data []byte) bool {
	d.hasher.Reset()
	d.hasher.Write([]byte(item))
	mainDigest := d.hasher.Sum(nil)
