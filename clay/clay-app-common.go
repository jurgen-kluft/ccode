		...
		for _, depPrj := range prjDependencyList.Values {
			if idx, ok := projectNameToIndex[depPrj.Name]; ok {
				...
				if !depPrj.HasMatchingConfigForTarget(buildConfig, buildTarget) {
					//corepkg.LogInfof("Dependency project %s does not have a matching config for target %s and build %s, skipping", depPrj.Name, buildTarget.String(), buildConfig.String())
					continue
				}
