...
func Build(projectName string, buildConfig denv.BuildConfig, buildTarget denv.BuildTarget) (err error) {
	...
	if clayConfig.TargetOs == "arduino" {
		if clayConfig.TargetArch == "esp32" && len(clayConfig.TargetBoard) > 0 {
			...
			if board == nil {
				return fmt.Errorf("Esp32 board not found: %s", clayConfig.TargetBoard)
			}
			esp32Toolchain.ResolveVariables(board, buildPath)
		} else if clayConfig.TargetArch == "esp8266" && len(clayConfig.TargetBoard) > 0 {
			esp8266Toolchain := NewEsp8266Toolchain()
			if err := LoadToolchainJson(esp8266Toolchain, "esp8266.json"); err != nil {
				return err
			}
			board = esp8266Toolchain.GetBoardByName(clayConfig.TargetBoard)
			if board == nil {
				return fmt.Errorf("Esp8266 board not found: %s", clayConfig.TargetBoard)
			}
			esp8266Toolchain.ResolveVariables(board, buildPath)
		} else {
			return fmt.Errorf("Architecture '%s' not found with board '%s'", clayConfig.TargetArch, clayConfig.TargetBoard)
		}
	}
...
func CreateProjects(buildTarget denv.BuildTarget, buildConfig denv.BuildConfig) ([]*Project, error) {
	...
	// for _, prj := range pkg.GetMainLib() {
	// }

	// TODO glob all the source files for each project (filtered by build target and config)
