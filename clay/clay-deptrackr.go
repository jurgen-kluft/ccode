...
import (
	...
	"github.com/jurgen-kluft/ccode/clay/deptrackr"
)
...
type DepTrackr struct {
	Current      deptrackr.ReadableDepTracker
	CurrentState deptrackr.State
	New          deptrackr.WriteableDepTrackr
	Hasher       hash.Hash
}

func LoadDepTrackr(buildDir string) *DepTrackr {
	current := deptrackr.Load(buildDir)
	tracker := current.NewDB()
	return &DepTrackr{
		Current:      current,
		CurrentState: deptrackr.StateUpToDate, // Start with an up-to-date state
		New:          tracker,
		Hasher:       sha1.New(),
	}
}
...
func (d *DepTrackr) Save() error {
	if d.CurrentState == deptrackr.StateUpToDate {
...
func (d *DepTrackr) QueryFile(srcfileAbsFilepath string) bool {
	...
	state, err := d.Current.QueryItem(mainDigest, true, func(itemChangeFlags uint16, itemChangeData []byte, itemIdFlags uint16, itemIdData []byte) deptrackr.State {
		if itemIdFlags&deptrackr.ItemFlagSourceFile == deptrackr.ItemFlagSourceFile || itemIdFlags&deptrackr.ItemFlagDependency == deptrackr.ItemFlagDependency {
			...
			if err == nil {
				...
				if fileModCompare(fileModTimeData, itemChangeData) {
					return deptrackr.StateUpToDate
				}
			}
			return deptrackr.StateOutOfDate
		} else {
			// This is not a source file or dependency file, so we ignore it
			return deptrackr.StateIgnore
		}
	})
	...
	if state == deptrackr.StateUpToDate {
		return true
	} else if state == deptrackr.StateOutOfDate {
		d.CurrentState = deptrackr.StateOutOfDate
	}
...
func (d *DepTrackr) AddFile(srcfileAbsFilepath string, depfileAbsFilepath string) error {
	...
	item = deptrackr.ItemToAdd{
		...
		IdDigest:     mainHash,
		IdFlags:      deptrackr.ItemFlagDependency,
		ChangeData:   modTimeBytes,
		ChangeDigest: nil, // mod-time is small enough, we do not need a hash
		ChangeFlags:  deptrackr.ChangeFlagModTime,
	}
	depItems := []deptrackr.ItemToAdd{item}
...
func (d *DepTrackr) AddFileAndDependencies(fileAbsFilepath string, listOfFileDependencies []string, listOfStringDependencies []StringDependency) error {
	...
	for _, p := range listOfStringDependencies {
		...
		depItem := deptrackr.ItemToAdd{
			IdDigest:     depDigest,
			IdData:       []byte(p.Main),
			IdFlags:      deptrackr.ItemFlagString,
			ChangeDigest: contentDigest,
			ChangeData:   []byte(p.Content),
			ChangeFlags:  deptrackr.ChangeFlagString,
		}
		depItems = append(depItems, depItem)
	}
