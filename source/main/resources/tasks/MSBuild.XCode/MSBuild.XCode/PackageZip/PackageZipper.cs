...
namespace MSBuild.XCode
{
    ...
    /// </summary>
    public class PackageZipper : IDisposable
    {
        /// <summary>
        ...
        private List<ZipFileEntry> mFiles = new List<ZipFileEntry>();
        ...
        public List<ZipFileEntry> Files
        {
            get
            {
                return mFiles;
        ...
        public static PackageZipper Create(string _filename, string _comment)
        {
            Stream stream = new FileStream(_filename, FileMode.Create, FileAccess.ReadWrite);

            PackageZipper zip = Create(stream, _comment);
        ...
        /// <returns>A valid ZipStorer object</returns>
        public static PackageZipper Create(Stream _stream, string _comment)
        {
            PackageZipper zip = new PackageZipper();
            zip.Comment = _comment;
        ...
        /// <returns>A valid ZipStorer object</returns>
        public static PackageZipper Open(Stream _stream, FileAccess _access)
        {
        ...
        {
            AddFile(Compression.Deflate, _pathname, _filenameInZip, string.Empty);
        }
        ...
        public void AddStream(Compression _method, string _filenameInZip, Stream _source, DateTime _modTime, string _comment)
        {
            ...
            {
                ZipFileEntry last = this.mFiles[this.mFiles.Count-1];
            ...
            if (this.Access != FileAccess.Read)
            {
                if (this.ZipFileStream != null)
                {
                    ...
                    for (int i = 0; i < mFiles.Count; i++)
                    {
                        ...
                        this.WriteCentralDirRecord(mFiles[i]);
        ...
        public bool FindEntryByFilename(string filename, out ZipFileEntry outEntry)
        {
            foreach (ZipFileEntry e in mFiles)
            {
                if (e.FilenameInZip == filename)
                {
                    outEntry = e;
                    return true;
                }
            }
            outEntry = new ZipFileEntry();
            return false;
        }
        ...
        public bool ExtractTo(string path)
        {
            ...
            progress.Add(mFiles.Count);
            foreach (ZipFileEntry e in mFiles)
            {
                string filepath = path + e.FilenameInZip;
                if (!ExtractFile(e, filepath))
                {
                    failedEntries.Add(e);
        ...
        public static bool RemoveEntries(ref PackageZipper _zip, List<ZipFileEntry> _zfes)
        {
            ...
            List<ZipFileEntry> fullList = _zip.mFiles;
            ...
            try
            {
                PackageZipper tempZip = PackageZipper.Create(tempZipName, string.Empty);
                ...
                _zip = PackageZipper.Open(_zip.FileName, _zip.Access);
            }
            catch
            {
                return false;
        ...
        */
        private void WriteEndRecord(uint _size, uint _offset)
        {
            ...
            this.ZipFileStream.Write(BitConverter.GetBytes((ushort)mFiles.Count+ExistingFiles), 0, 2);
            this.ZipFileStream.Write(BitConverter.GetBytes((ushort)mFiles.Count+ExistingFiles), 0, 2);
        ...
        private void Store(ref ZipFileEntry _zfe, Stream _source)
        {
            ...
            do
            {
                ...
                if (bytesRead > 0)
                {
                    ...
                    {
                        _zfe.Crc32 = PackageZipper.CrcTable[(_zfe.Crc32 ^ buffer[i]) & 0xFF] ^ (_zfe.Crc32 >> 8);
                    }
                }
