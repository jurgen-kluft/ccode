...
namespace FileDirectoryPath
{
    ...
    public abstract class BasePath
    {
        ...
        //  Ctor
        //
        public BasePath(string path, bool absolute)
        {
            if (path == null)
            {
            ...
            path = InternalStringHelper.NormalizePath(path, absolute);
            ...
            if (InternalStringHelper.ContainsInnerSpecialDir(path, absolute))
            {
                InternalStringHelper.TryResolveInnerSpecialDir(path, absolute, out path, out errorReason);
            }
        ...
        public abstract bool IsNetworkPath { get; }
        ...
        protected static string GetPathRelative(DirPathAbsolute pathFrom, BasePath pathTo)
        {
            ...
            {
                throw new ArgumentException(@"Cannot compute relative path from 2 paths that are not on the same drive PathFrom = """ + pathFrom.Path + @"""PathTo   = """ + pathTo.Path + @"""");
            }
