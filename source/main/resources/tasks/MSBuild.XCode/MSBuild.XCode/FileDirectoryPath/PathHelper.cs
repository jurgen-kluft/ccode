...
namespace FileDirectoryPath
{
    public static class PathHelper
    {
        ...
        public static bool IsValidAbsolutePath(string path, out string reason)
        {
            ...
            if (InternalStringHelper.IsUniformResourceLocatorPath(path))
            {
                reason = "URN (UniformRessourceLocator) paths are not supported.";
                return false;
            }

            if (!InternalStringHelper.BeginsWithASingleLetterDrive(path) && !InternalStringHelper.BeginsWithDoubleSlashNetworkDrive(path))
            {
                reason = "The absolute path doesn't begin with a single letter drive followed by ':' or as a double slash network drive.";
                return false;
            }

            path = InternalStringHelper.NormalizePath(path, true);
            if (InternalStringHelper.ContainsInnerSpecialDir(path, true))
            {
                string unusedPath;
                if (!InternalStringHelper.TryResolveInnerSpecialDir(path, true, out unusedPath, out reason))
                {
                    return false;
                }
            }
        ...
        public static bool IsValidRelativePath(string path, out string reason)
        {
            ...
            path = InternalStringHelper.NormalizePath(path, false);
            if (InternalStringHelper.ContainsInnerSpecialDir(path, false))
            {
                string unusedPath;
                if (!InternalStringHelper.TryResolveInnerSpecialDir(path, false, out unusedPath, out reason))
                {
                    return false;
                }
            }
        ...
        public static string TryRemoveDllOrExeExtension(string filePath)
        {
            ...
            if (string.Compare(lastFourChars, ".dll", true) == 0 || string.Compare(lastFourChars, ".exe", true) == 0)
            {
