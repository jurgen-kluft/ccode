...
namespace FileDirectoryPath
{
    internal static class InternalStringHelper
    {
        ...
        // .\..\Dir2\.\Dir3
        // .\..\..\Dir2\..\Dir3
        internal static bool ContainsInnerSpecialDir(string path, bool absolute)
        {
            // These cases should have been handled by the calling method and cannot be handled
            Debug.Assert(path != null);
            Debug.Assert(path.Length != 0);
            Debug.Assert(path == NormalizePath(path, absolute));

            // Analyze if a /./ or a /../ donc come after a valid DirectoryName
            string[] pathDirs = path.Split(new char[] { DIR_SEPARATOR_CHAR }, StringSplitOptions.RemoveEmptyEntries);
        ...
        internal static bool TryResolveInnerSpecialDir(string path, bool absolute, out string pathResolved, out string failureReason)
        {
            // These cases should have been handled by the calling method and cannot be handled
            Debug.Assert(path != null);
            Debug.Assert(path.Length != 0);
            ...
            Debug.Assert(ContainsInnerSpecialDir(path, absolute));
            ...
            Debug.Assert(pathResolved == NormalizePath(pathResolved, absolute));
            return true;
        }
        ...
        //  Path normalization
        //
        internal static string NormalizePath(string path, bool absolute)
        {
            Debug.Assert(path != null && path.Length > 0);
            path = path.Replace('/', DIR_SEPARATOR_CHAR);

            bool isNetworkPath = absolute && path.StartsWith(@"\\");
            ...
            // Restore double slash for network path
            if (isNetworkPath)
                path = path.Insert(0, "\\");
        ...
        internal static string GetPathRelativeTo(string pathFrom, string pathTo)
        {
            ...
            string[] pathFromDirs = pathFrom.Split(new char[] { DIR_SEPARATOR_CHAR }, StringSplitOptions.RemoveEmptyEntries);
            string[] pathToDirs = pathTo.Split(new char[] { DIR_SEPARATOR_CHAR }, StringSplitOptions.RemoveEmptyEntries);
...
PathTo   = """ + pathTo + @"""");
        ...
        //
        internal static bool IsUniformResourceLocatorPath(string path)
        {
        ...
        internal static bool BeginsWithDoubleSlashNetworkDrive(string path)
        {
            if (path.Length < 2)
            {
                return false;
